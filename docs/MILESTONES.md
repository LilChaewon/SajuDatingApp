...
- [ ] **M1.5: Python 실행 환경 구성:** 서버에 Python, TensorFlow, numpy 등 궁합 점수 계산 로직 실행에 필요한 라이브러리 환경 구성.
- [ ] **M2: 사용자 인증 및 프로필 API 개발:** JWT 기반 회원가입, 로그인, 로그아웃 기능 구현. 사용자 프로필 조회 및 수정 API 엔드포인트 개발.
- [ ] **M3: 사주 분석 및 운세 API 개발:** `legacy/오늘의운세로직`의 데이터 조회 및 '십성' 계산 로직을 Node.js로 포팅하고, 오늘의 운세 제공 API 개발.
- [ ] **M4: 핵심 매칭 및 궁합 API 개발:** Node.js에서 `legacy/궁합점수로직`의 Python 스크립트를 실행하고 결과를 반환하는 래퍼(wrapper)를 개발. 사용자 추천 알고리즘 및 궁합 점수 계산 API 개발.
